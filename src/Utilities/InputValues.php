        ...
        if($type->isObjectType()){
            ...
            foreach (array_keys($inputValue) as $fieldName) {
                if(!array_key_exists($fieldName, $fieldDefs)){
                    //TODO: build suggestions (see: https://github.com/graphql/graphql-js/blob/5ed55b89d526c637eeb9c440715367eec8a2adec/src/utilities/coerceInputValue.js#L131)
                    throw new GraphQLError(
                        "Field \"{$fieldName}\" is not defined by type \"{$type->getName()}\""
                    );
                }
            }
            return $coercedValue;
